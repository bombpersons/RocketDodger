...
/*
	...
	// Private vars
	MeshFilter filter;
	LinkedList<BarPoint> points = new LinkedList<BarPoint>();
	...
	void Update () {
		...
		if (Player == null) {
			...
			if (cooldown < 0) {
				...
				BarPoint left = new BarPoint();
				left.Normal = normal;
				left.Forwards = outdir;
				left.Pos = new Vector2(Player.position.x, Player.position.z) + normal * StartWidth;
				left.Speed = DispersionRate + Random.Range(-0.5f, 0.5f) * DispersionRate;
				left.BackSpeed = BackSpeed;// + Player.rigidbody.velocity.magnitude;
				BarPoint right = new BarPoint();
				right.Normal = normal;
				right.Forwards = outdir;
				right.Pos = new Vector2(Player.position.x, Player.position.z) - normal * StartWidth;
				right.Speed = -DispersionRate + Random.Range(-0.5f, 0.5f) * DispersionRate;
				right.BackSpeed = BackSpeed;// + Player.rigidbody.velocity.magnitude;
